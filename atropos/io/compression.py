...
class GzipWriter:
    ...
    def close(self):
        ...
        if retcode != 0:
            raise IOError(
                "Output gzip process terminated with exit code {0}".format(retcode)
            )
...
FILE_OPENERS = {".gz": open_gzip_file, ".bz2": open_bzip_file, ".xz": open_lzma_file}
