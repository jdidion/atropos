...
class WorkerProcess(Process):
    ...
    def run(self):
        logging.getLogger().debug("%s running under pid %d", self.name, os.getpid())
...
def enqueue(
    queue,
    item,
    wait_message="Waiting to enqueue item {}",
    block_timeout=RETRY_INTERVAL,
    **kwargs
):
...
def dequeue(
    queue,
    wait_message="Waiting to dequeue item {}",
    block_timeout=RETRY_INTERVAL,
    **kwargs
):
