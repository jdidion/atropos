...
class SingleEndTrimPipeline(SingleEndPipelineMixin, TrimPipeline):
    pass


class PairedEndTrimPipeline(PairedEndPipelineMixin, TrimPipeline):
    pass


class ParallelSingleEndTrimPipeline(
    ParallelPipelineMixin, SingleEndPipelineMixin, TrimPipeline
):
    pass


class ParallelPairedEndTrimPipeline(
...
class CommandRunner(BaseCommandRunner):
    ...
    def __call__(self):
        ...
        if options.threads is None:
            ...
            if options.paired:
                pipeline_class = PairedEndTrimPipeline
            else:
                pipeline_class = SingleEndTrimPipeline
            ...
            return self.run_parallel(record_handler, writers)

    def run_parallel(self, record_handler, writers):
        ...
        result_queue = queue_manager.Queue(self.result_queue_size)
        writer_manager = None
        ...
        if self.writer_process:
            ...
            writer_manager = WriterManager(
                writers, compression, self.preserve_order, result_queue, timeout
            )
        else:
            worker_result_handler = WorkerResultHandler(
        ...
        if self.options.paired:
            pipeline_class = ParallelPairedEndTrimPipeline
        else:
            pipeline_class = ParallelSingleEndTrimPipeline
        pipeline = pipeline_class(record_handler, worker_result_handler)
        runner = ParallelTrimPipelineRunner(
            self, pipeline, threads, writer_manager, queue_manager
        )
        return runner.run()
