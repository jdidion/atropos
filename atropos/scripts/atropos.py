...
from atropos.filters import FilterType, FilterFactory, Filters
...
def main(cmdlineargs=None, default_outfile="-"):
	...
	if options.threads is None:
		...
		rc, summary = run_atropos_serial(reader, modifiers, filters, formatters, writers)
	else:
...
def get_option_parser():
	...
	group.add_option("-R", "--merge-overlapping", action="store_true", default=False,
		help="Merge read pairs that overlap into a single sequence. This is an experimental "
			 ...
			 "by --error-rate.")
	group.add_option("--merge-min-overlap", type=float, default=0.9,
		help="The minimum overlap between reads required for merging. If this number is (0,1.0], "
			 ...
			 "with an absolute minimum of 2 bp).")
	#group.add_option("--merge-mismatches", choices=("best", "N", "random"), default="best",
	...
	group.add_option("-i", "--cut-min", action='append', default=[], type=int, metavar="LENGTH",
		help="Similar to -u, except that cutting is done AFTER adapter trimming, and only "
			"if a minimum of LENGTH bases was not already removed")
	...
	group.add_option("--merged-output", default=None, metavar="FILE",
	...
	group.add_option("--log-level", default=None,
	...
	group.add_option("-I", "--cut-min2", action='append', default=[], type=int, metavar="LENGTH",
		help="Similar to -U, except that cutting is done AFTER adapter trimming, and only "
			"if a minimum of LENGTH bases was not already removed (see --cut-min)")
	...
	group.add_option("--bisulfite", default=False, metavar="METHOD",
		help="Set default option values for bisulfite-treated data. The argument specifies the "
			 ...
			 "is 1 or 0 for whether or not only trimmed reads should be further cut.")
	group.add_option("--mirna", action="store_true", default=False,
		help="Set default option values for miRNA data.")
	parser.add_option_group(group)
	
	group = OptionGroup(parser, "Parallel (multi-core) options")
	group.add_option("-T", "--threads", type=int, default=None, metavar="THREADS",
	...
	group.add_option("--process-timeout", type=int, default=30, metavar="SECONDS",
		help="Number of seconds process should wait before escalating messages to ERROR level.")
	group.add_option("--parallel-environment", choices=("local", "cluster"), default="local",
	...
	group = OptionGroup(parser, "Performance-tuning options")
	...
	group.add_option("--worker-compression", action="store_true", default=False,
		help="Perform data compression in the worker processes rather than the writer process.")
	parser.add_option_group(group)
...
def create_formatters(options, qualities, default_outfile):
	...
	if (options.merge_overlapping and options.merged_output):
		formatters.add_seq_formatter(FilterType.MERGED, options.merged_output)
		
	if (options.minimum_length is not None
			and options.minimum_length > 0
			and options.too_short_output):
		formatters.add_seq_formatter(FilterType.TOO_SHORT,
	...
	if options.maximum_length < sys.maxsize and options.too_long_output is not None:
		formatters.add_seq_formatter(FilterType.TOO_LONG,
			options.too_long_output, options.too_long_paired_output)
	
	if not formatters.multiplexed:
		if options.output is not None:
			formatters.add_seq_formatter(FilterType.NONE,
			...
			formatters.add_seq_formatter(FilterType.NONE, default_outfile)
			if default_outfile != "-":
	...
	if not options.discard_untrimmed:
		if formatters.multiplexed:
			...
			formatters.add_seq_formatter(FilterType.UNTRIMMED, untrimmed)
			formatters.add_seq_formatter(FilterType.NONE, untrimmed)
		elif options.untrimmed_output:
			formatters.add_seq_formatter(FilterType.UNTRIMMED,
...
def run_atropos_serial(reader, modifiers, filters, formatters, writers):
	try:
		n = 0
		total_bp1 = 0
		total_bp2 = 0
